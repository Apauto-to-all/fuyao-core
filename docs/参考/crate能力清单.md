...
- **公开 API**：
  ...
  - **选择支持类型**：`AgentIdOption`（`{ id, source }`，source 为 `AgentIdSource`：`Global` / `Workspace`）/ `DefinitionOption`（`{ id, definition }`，无来源字段——定义按文件名做优先级覆盖，同名互斥、高优先级层胜出）/ `ModelOption`（`{ id, provider, model }`）——供 fuyao-app 的 `Discovery` / `ProviderManager::list_models` / `list_agent_ids` 消费；`id` 为纯身份，agent_id 的层前缀由 `source` 独立承载
...
- **公开 API**：
  ...
  - **`ProviderManager`**（供应商管理门面，`FuyaoApp.providers` 字段，亦公开构造 `new(agent_paths)`——管理操作不依赖引擎装配）：供应商粒度的 CRUD 写回 **global 层**（`~/.fuyao/fuyao.toml` + `~/.fuyao/.env`），模型是供应商的组成内容（聚合成员），随供应商载荷整体写入与替换，不设独立的模型接口；toml 用增量 patch（只动目标 `[providers.<id>]` 子树，注释 / 未知字段 / 手写格式逐字保留，段内 `models` 子表按载荷整表替换——未携带的模型消失、载荷 id 即落盘键，模型 id 可随替换变更），api_key 明文只进 `.env` 单行 upsert（同名覆盖、异名新加），toml 只写 `api_key_env_vars` 指针（变量名用户自设 `api_key_env_var`，与供应商 id 解耦，单值所见即所得）；.env 只增改不删除——变量与值均归用户持有，删除供应商也不清理 .env；供应商 id 创建后不可改名（API 签名以 id 定位，换 id 走「建新（沿用原变量名，.env 密钥行不动）+ 删旧」）；`create_provider(id, ProviderSpec)`（载荷一次落齐全部字段与模型）/ `update_provider(id, ProviderSpec)`（管理字段 patch + `models` 整表替换；`api_key = None` 不动 .env，`Some` 时 upsert 明文并移除 toml 残留明文）/ `delete_provider(id)`（toml 段级联删全部模型，.env 不动）/ `list_models()` → `Vec<ModelOption>`（Provider 注册缓存，启动前为空，`provider_name` 注入 + `(provider_id, id)` 双层字母序）/ `list_providers_with_source()`（三层落盘直读，带来源层标注）；写盘结果经 `load_config` 读回语义一致；内存注册与 registry 刷新不在其职责内

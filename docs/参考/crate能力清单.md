...
- **公开 API**：
  ...
  - **`ProviderManager`**（供应商管理门面，`FuyaoApp.providers` 字段，亦公开构造 `new(agent_paths)`——管理操作不依赖引擎装配）：供应商与模型粒度的 CRUD 写回 **global 层**（`~/.fuyao/fuyao.toml` + `~/.fuyao/.env`），toml 用增量 patch（只动目标 `[providers.<id>]` 子树，注释 / 未知字段 / 手写格式逐字保留），api_key 明文只进 `.env` 单行（变量名按 `{供应商 id 大写}_API_KEY` 约定生成，目标变量已有不同值返回 `EnvKeyConflict` 冲突信号，`overwrite_api_key = true` 确认覆盖后重试），toml 只写 `api_key_env_vars` 指针；id 创建后不可改名（API 签名以 id 定位，改名以删旧建新满足）；`create_provider(id, ProviderSpec)` → 环境变量名（返回值供表单只读展示）/ `update_provider(id, ProviderSpec)`（`name` / `base_url` 完整期望状态覆盖，`api_key = None` 不动 .env，`Some` 时同步指针并移除 toml 残留明文）/ `delete_provider(id)`（级联删全部模型 + 同步清理 .env 约定变量行）/ `create_model(provider_id, model_id, Model)` / `update_model(provider_id, model_id, Model)`（整表替换目标模型段）/ `delete_model(provider_id, model_id)` / `list_models()` → `Vec<ModelOption>`（Provider 注册缓存，启动前为空，`provider_name` 注入 + `(provider_id, id)` 双层字母序）；写盘结果经 `load_config` 读回语义一致；内存注册与 registry 刷新不在其职责内
  - **`ProviderSpec`**（供应商写回载荷）：`{ name, base_url: Option<String>, api_key: Option<String>, overwrite_api_key: bool }`（Debug 输出屏蔽 api_key）
  - **`LogGuard`**：drop 时 flush 文件日志
  - **错误**：`InitError`（`NoProviderAvailable` / `ConfigError` / `InvalidAgentId`）/ `SetupError`（`Init`）/ `ProviderAdminError`（`Invalid` 校验失败 / `AlreadyExists` 创建冲突 / `NotFound` 目标缺失 / `EnvKeyConflict` .env 变量已有值未确认覆盖 / `TomlParse` / `InvalidSection` / `Io`）

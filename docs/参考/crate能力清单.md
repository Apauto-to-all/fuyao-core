...
- **公开 API**：
  ...
  - **选择支持类型**：`AgentIdOption`（`{ id, source }`，source 为 `AgentIdSource`：`Global` / `Workspace`）/ `DefinitionOption`（`{ id, definition }`，无来源字段——定义按文件名做优先级覆盖，同名互斥、高优先级层胜出）/ `ProviderOption`（`{ id, name, source, base_url, api_key_env_vars, models }`）与 `ProviderModelOption`（`{ id, source, model }`，source 为 `ProviderSource`：`Global` / `Agent` / `Workspace`）——供 fuyao-app 的 `Discovery` / `ProviderManager::list_providers` / `list_agent_ids` 消费；`id` 为纯身份，agent_id 的层前缀由 `source` 独立承载
...
- **职责**：装配入口 + fan-in 单一出口：`start` / `init_engine` / `build_tool_registry` / `App` / `SessionManager` / `Discovery` / `ProviderManager` / `list_agent_ids`
- **内部依赖**：api, core, guard, hooks, mcp, provider, prompt, session, tools
- **公开 API**：
  - **`start(EngineParams)`**：一行启动（`init_engine` → `build_tool_registry` → 装配 `LoopGuardPlugin` → 创建 `SessionStore` → `Engine::new`（注入 store）→ `App::new` → `SessionManager::new` → `Discovery::new` → `ProviderManager::new`），返回 `FuyaoApp { app, sessions, discovery, providers }`——上层同时拿到运行时入口（`app`）、查询入口（`sessions`）、选择支持入口（`discovery`）、供应商管理入口（`providers`）；`app` 与 `sessions` 共享同一份 `Arc<SessionStore>`
  - **`FuyaoApp`**（`start` 的聚合产物）：`app: App`（运行时交互：create/send/stop/recv/end）+ `sessions: SessionManager`（会话检索：list/count）+ `discovery: Discovery`（选择支持：列 Agent 定义）+ `providers: ProviderManager`（供应商管理：列 model、供应商与模型 CRUD 写回），平级正交、互不依赖
  - **`App`**（运行时交互门面，持 `Engine` + fan-in 出口）：`new(engine, mcp_manager, log_guard)` / `create_session(SessionParams)` → `SessionId`（rx 由内部 forwarder 消费进 fan_out）/ `resume_session` / `fork_session` / `create_child_session(parent, source, params)` → `(SessionId, rx)`（**rx 不进 fan_out**，返调用方独占消费）/ `send` / `stop_session(id, reason)`（屏障停 turn，直接代理 `Engine::stop_session`；返回即该 session 的 DB 已静默，session 保持存活。管理型同步方法，不走消息总线；「先停后改库」两步编排（如回退：先 stop 再 `SessionManager::rollback_session`）的第一步）/ `register_provider(provider_id)`（向存活 engine 注入一个供应商，直接代理 `Engine::register_provider`：配置进注册缓存 + 实例进注册表，下一 turn 立即可用；同步执行，多 engine 场景的全池刷新编排属上层职责）/ `unregister_provider(provider_id)` → `bool`（从存活 engine 移除一个供应商，直接代理 `Engine::unregister_provider`：实例与缓存条目含旗下模型一并清除；false = 本就不在注册表，幂等）/ `recv()` → `Option<OutputEvent>`（单一出口）/ `end_session` / `shutdown(self)`（两段式：engine.shutdown → forwarder 退出 → 停 MCP → drop log_guard）
  ...
  - **`Discovery`**（选择支持门面，`FuyaoApp.discovery` 字段，持 `start` 注入的 `AgentPaths`）：`list_primary_definitions()` → `Vec<DefinitionOption>`（会话人格专用，仅 Primary 模式，四层定义目录 + 内置，零参数）/ `list_subagent_definitions()` → `Vec<DefinitionOption>`（子代理工具候选全集，仅 Subagent 模式，与主代理列举按 mode 互斥）；路径身份启动时注入一次，所有查询共用，调用方不再传参
  - **`ProviderManager`**（供应商管理门面，`FuyaoApp.providers` 字段，亦公开构造 `new(agent_paths)`——管理操作不依赖引擎装配）：供应商粒度的 CRUD 写回 **global 层**（`~/.fuyao/fuyao.toml` + `~/.fuyao/.env`），模型是供应商的组成内容（聚合成员），随供应商载荷整体写入与替换，不设独立的模型接口；toml 用增量 patch（只动目标 `[providers.<id>]` 子树，注释 / 未知字段 / 手写格式逐字保留，段内 `models` 子表按载荷整表替换——未携带的模型消失、载荷 id 即落盘键，模型 id 可随替换变更），api_key 明文只进 `.env` 单行 upsert（同名覆盖、异名新加），toml 只写 `api_key_env_vars` 指针（变量名用户自设 `api_key_env_var`，与供应商 id 解耦，单值所见即所得）；.env 只增改不删除——变量与值均归用户持有，删除供应商也不清理 .env；供应商 id 创建后不可改名（API 签名以 id 定位，换 id 走「建新（沿用原变量名，.env 密钥行不动）+ 删旧」）；`create_provider(id, ProviderSpec)`（载荷一次落齐全部字段与模型）/ `update_provider(id, ProviderSpec)`（管理字段 patch + `models` 整表替换；`api_key = None` 不动 .env，`Some` 时 upsert 明文并移除 toml 残留明文）/ `delete_provider(id)`（toml 段级联删全部模型，.env 不动）/ `list_providers()` → `Result<Vec<ProviderOption>>`（**唯一列举接口**：三层落盘直读、带来源层标注、写盘即见、引擎启动前可用，供应商与组内模型双层 id 字母序；坏配置 fail-loud）；写盘结果经 `load_config` 读回语义一致；内存注册与 registry 刷新不在其职责内
  - **`ProviderSpec`**（供应商写回载荷，create / update 共用的完整期望状态）：`{ name, base_url: Option<String>, api_key_env_var: Option<String>（用户自设 .env 变量名，明文提供时必填）, api_key: Option<String>（None 不动 .env）, models: Vec<ProviderModelSpec{ id, model: Model }>（全量模型列表，update 时整表替换）}`（Debug 输出屏蔽 api_key）
  - **`LogGuard`**：drop 时 flush 文件日志
  - **错误**：`InitError`（`NoProviderAvailable` / `ConfigError` / `InvalidAgentId`）/ `SetupError`（`Init`）/ `ProviderAdminError`（`Invalid` 校验失败 / `AlreadyExists` 创建冲突 / `NotFound` 目标缺失 / `TomlParse` / `InvalidSection` / `Io`）

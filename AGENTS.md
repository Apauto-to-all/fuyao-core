...
- **供应商定义单点**：`[providers]` 段只允许出现在全局层 `fuyao.toml`（单一事实源），agent / workspace 层出现即加载报错；管理面（CRUD）与加载面读写同一份落盘。分层的是「选择」（model_id / `[llm]` 配置走三层合并），单点的是「定义」
...
- **fuyao-app 导出面是对外契约**：fuyao-app 是外部二次开发应用（cli / tui 等）的唯一依赖，公开导出（门面、透出类型、分步装配函数）的消费者在仓库之外——仓库内无调用方不等于无人使用，纯转发性质的门面与透出的便利类型都是对外 API，禁止以「内部无人用 / 简化」为由删除
...
| 概念 | 本质 | 存储位置 | 决定什么 |
| ------ | ------ | --------- | --------- |
| **agent_id** | 独立 Agent 实体（数据隔离单元） | `fuyao-agents/{id}/`（一个目录） | 数据在哪：sessions.db、缓存 |
...
- 注释只描述代码本身：禁止在代码注释里出现任何"出处/对标"话语——不论动词（参考/对齐/学/借鉴/移植/搬自/对应…，不穷举）其他文件、文档、项目、链接等干扰话语。设计依据要写就重述为代码自身的设计陈述，不带出处。
- 禁止变更历史注释（辩护词）：注释只描述当前存在的代码，禁止出现任何关于"变更本身"的陈述——为什么删除 / 为什么移动 / 已迁至 / 已移出 / 不再属于 / 原先在 等一律禁止。删除即删除、移动即移动，原位置一个字不留；删除/移动的原因只写进提交消息正文。注释的受众是未来读代码的人，不是本次改动的 review 者。改完 .rs 必须自检注释是否命中上述两条禁项。
...
- 同步内置skills：凡代码改动涉及内置 skills（`crates/fuyao-prompt/src/builtin/assets/skills/`）所描述的事实——配置键、工具集、行为语义、协议流程等——必须同步更新对应 skill 的描述，禁止只改代码不改文档

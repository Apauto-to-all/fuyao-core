...
| `docs/教程/` | 教程 |
| `docs/指南/` | 怎么做某件事 |
| `docs/参考/` | 查事实（配置项、事件协议、错误类型、工具参数、crate 清单） |
| `docs/解释/` | 理解为什么这么设计 |

**快速上手路径**：`docs/解释/`（架构全貌）→ `docs/参考/`（查具体字段）→ 按需深入对应 crate 源码。

> 改架构前先读 `docs/解释/` 下对应子系统的设计文档，理解现有决策的背景。
...
- 不调用 question 工具，如果有任何疑问，直接在对话中询问

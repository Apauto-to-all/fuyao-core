...
//! 内存注册缓存与存活 engine 的 registry 刷新属「立即生效」能力，不在本门面
//! 职责内（由引擎装配层另行编排）。

use std::collections::HashMap;
use std::fmt;

use fuyao_api::{AgentPaths, Model, ModelOption};
...
impl ProviderManager {
    ...
    /// 下拉等）可直接沿用本序呈现。
    ///
    /// 本列表反映**注册缓存**（引擎装配时写入）；经本门面写盘的新增供应商要
    /// 进入列表需等注册缓存刷新（重新装配或运行时注册），写盘与列举是两条
    /// 独立路径。

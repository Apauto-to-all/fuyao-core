...
文件快照（回退的文件侧联动）：引擎在工具批执行前与 turn 收尾对工作区做影子 git 快照，回退会话消息时把 AI 改过的文件一并退回回退点状态（修改恢复、新建删除）。

| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `enabled` | bool | `true` | 总开关：`false` 时全程跳过采集，消息回退照常、文件不动 |
| `max_untracked_mb` | 整数 | `2` | 未跟踪文件入快照的大小上限（MB），超限文件排除出快照仓 |

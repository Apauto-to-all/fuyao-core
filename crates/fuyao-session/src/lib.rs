...
//! + 上下文压缩（[`compressor`]：阈值检测 + 摘要生成；落库交调用方直接调
//!   [`SessionStore::mark_compaction`]）
...
//! - `compressor`：上下文压缩（触发 / 摘要生成；落库由调用方直接调 store）
...
pub use store::{CompressionReason, RollbackResult, SessionStore};

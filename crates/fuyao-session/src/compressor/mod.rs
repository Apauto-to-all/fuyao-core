...
//! - 三层切分：触发（纯函数）/ 执行（异步调 LLM + 窗口切分）/ 落地（调 store）
//!
//! 文件组织：
//! - [`trigger`]：触发层（阈值检测 + 反抖动）
//! - [`window`]：窗口算法（保留 tail 的 token 预算切分 + 整 turn 完整性）
//! - [`summary`]：执行层（构造 prompt + 调 provider + 失败处理）
//! - [`prompt`]：摘要 system prompt + previous-summary 注入模板
//! - [`apply`]：落地层（调 store.mark_compaction + 内存重建）
...
pub use trigger::{CompressionState, should_compress};

        ...
        let action = {
            ...
            match event {
                ...
                InputEvent::Control(ctrl_msg) => {
                    // 入口转化由类型方法承载（input → output 侧同构消息，逐字段
                    // 搬运）；包成 Control 条目走与用户消息相同的入站通道——
                    // 同一通道承载保证两类消息的总序。命令本体（如手动压缩跳过
                    // 阈值，reason=manual）由消费点的 handle_control 先回显后执行，
                    // client_message_id 供排队中撤销与消费回显配对，note（附言）
                    // 随回显原样携带。
                    OutboundAction::Inbound(
                        handle.tx_inbound.clone(),
                        QueueEntry::Control(ctrl_msg.into_output()),
                    )
                }

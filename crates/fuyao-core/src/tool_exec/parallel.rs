...
//!
//! 本模块是纯逻辑（不依赖 provider/task 运行时），便于独立单元测试。
...
/// 工具调用信息（轻量级，用于并行判断）
///
/// 由调用方从 `ToolCallData` 映射而来，解耦并行判断与 provider 类型。
...
/// 3. path_scoped_tools 检查路径重叠 → 重叠或取不到路径则串行
...
/// 从工具参数中提取路径
///
/// 读取 `args.path` 字段，处理 `~` 居家目录展开，规范化为绝对路径。
/// 取不到路径（缺失/空/非字符串）返回 None。

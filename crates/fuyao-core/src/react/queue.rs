...
//! 每条经统一历史入口（`crate::history::inject_user_messages`：拦截 → 落 DB →
//! 发送事件 → 观察），与 assistant / tool_result 走完全相同的管道。
//!
...
pub(crate) fn consume_all_guide(
    guide: &SharedQueue,
) -> Vec<fuyao_api::message::output::UserMessage> {

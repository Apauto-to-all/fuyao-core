...
mod tests {
    ...
    async fn execute_command_timeout() {
        let shell_info = super::super::shell::find_shell();
        // 仅验证超时语义：1 秒超时 vs ping 要跑约 5 秒。
        // 不重定向输出：stdout 本就被 Stdio::piped() 接到内存，不会落盘；
        // 且避免 Git Bash 对 "> NUL" 把 NUL 当普通文件名、对 "> /dev/null" 做路径转换
        // 这两类 shell 差异干扰被测语义。
    ...
    async fn execute_command_cancelled() {
        ...
        let shell_info = super::super::shell::find_shell();
        // 同 execute_command_timeout：不重定向输出，避免 Git Bash 的 NUL/路径转换差异
        let command = if cfg!(windows) {
            "ping -n 6 127.0.0.1"
        } else {
            "sleep 5"

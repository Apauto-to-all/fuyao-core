...
/// bash 工具基础描述（已定稿文案，除按 shell 追加语法提示外不改）
const BASH_DESCRIPTION_BASE: &str = "在本地终端执行 shell 命令（构建、测试、git、进程管理等）";
...
mod tests {
    ...
    /// bash_description：Windows 系 shell 在末尾追加语法提示，基础文案保持原样
    #[test]
    fn bash_description_appends_hint_for_windows_shells() {
        ...
        assert_eq!(
            ...
            "在本地终端执行 shell 命令（构建、测试、git、进程管理等）命令经 PowerShell 执行。"
        );
        assert_eq!(
            bash_description("cmd"),
            "在本地终端执行 shell 命令（构建、测试、git、进程管理等）命令经 CMD 执行（路径用 \\）。"
        );
    }

    /// bash_description：bash / sh 与 Unix 原生假设一致，描述即基础文案
    #[test]
